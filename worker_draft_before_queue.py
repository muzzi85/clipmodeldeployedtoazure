...
def classify_room_from_objects(objects_dict):
    ...
    livingroom_objects = ["a sofa", "a television", "a dinning table and chairs", "a heater radiator"]
...
# def detect_objects_multiscale_annotate(image_path, n_scales=4, overlap_ratio=0.1, confidence_threshold=0.7):
#     """Multi-scale object detection with annotation."""
...
#         if x_steps[-1] + crop_size < W: x_steps.append(W - crop_size)
#         if y_steps[-1] + crop_size < H: y_steps.append(H - crop_size)
#         for top in y_steps:
#             for left in x_steps:
...
#                     center_x, center_y = left + crop_size // 2, top + crop_size // 2
...
# =========================================================
# 5. PROCESS ALL PROPERTIES
# =========================================================
def process_all_properties(input_dir, output_dir):
    os.makedirs(output_dir, exist_ok=True)

    for property_folder in os.listdir(input_dir):
        src_path = os.path.join(input_dir, property_folder)
        dst_path = os.path.join(output_dir, property_folder)
        os.makedirs(dst_path, exist_ok=True)
        if not os.path.isdir(src_path): continue
        print(f"📁 Processing {property_folder}")
        ...
        for file in os.listdir(src_path):
            ...
            try:
                ...
                w, h = img.size
                if w < 300 or h < 400: continue
            except Exception as e:
                print(f"⚠️ Failed {file}: {e}")
                continue

            # Multi-scale detection
            annotated_img, labels_dict = detect_objects_multiscale_annotate(
                image_path, n_scales=4, overlap_ratio=0.15, confidence_threshold=0.85
            )

            # CLIP classification
            label, conf, clean_level, reason = classify_room_and_cleanliness(img, object_conf_threshold=0.9)
            if conf is None or conf < 0.9:
                label="unknown"; clean_level="None"; reason="None"; conf="None"
            ...
            json_path = os.path.join(dst_path, file.rsplit(".",1)[0]+".json")
...
def main():
    ...
    process_all_properties(INPUT_DIR, OUTPUT_DIR)
    print("🎉 Job completed")
